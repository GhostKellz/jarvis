use anyhow::Result;
use jarvis_nvim::{JarvisNvim, AIIntegration, lsp::JarvisLspServer};
...
async fn main() -> Result<()> {
    ...
    match args[1].as_str() {
        ...
        "socket" => {
            let socket_path = args.get(2).map(|s| s.as_str()).unwrap_or("/tmp/jarvis.sock");
            ...
            _ => {
                ...
                let response = match parts[0] {
                    "explain" => {
                        ...
                        ai.explain_code(code, "rust", "").await.unwrap_or_else(|e| format!("Error: {}", e))
                    }
                    "improve" => {
                        let code = parts.get(1).unwrap_or(&"");
                        ai.suggest_improvements(code, "rust").await.unwrap_or_else(|e| format!("Error: {}", e))
                    }
                    "fix" => {
                        let code = parts.get(1).unwrap_or(&"");
                        ai.fix_errors(code, &[], "rust").await.unwrap_or_else(|e| format!("Error: {}", e))
                    }
                    "generate" => {
                        let description = parts.get(1).unwrap_or(&"");
                        ai.generate_code(description, "rust", "").await.unwrap_or_else(|e| format!("Error: {}", e))
                    }
                    "chat" => {
                        let message = parts.get(1).unwrap_or(&"");
                        ai.send_message(message, None).await.unwrap_or_else(|e| format!("Error: {}", e))
                    }
                    "quit" | "exit" => break,
